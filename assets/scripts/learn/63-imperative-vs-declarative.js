...
(() => {
  ...
  (() => {
    ...
    foundUser = users.find((user) => user.name === "나설");
  ...
  (() => {
    ...
    console.table(korean); // 새로운 집단은 국적이 추가된 사용자들로 구성됨
    console.table(users); // 원본 users 배열은 변경되지 않음
  })();
  ...
  (() => {
    const descListTemplate = `
      <dl>
        ${techTerms.reduce((acc, { term, description }) => {
          return `
            ${acc}
            <dt>${term}</dt>
            <dd>${description}</dd>
  ...
  (() => {
    ...
    const subjectCount = Object.values(timetable)
      .flat()
      .reduce((acc, subject) => {
        // 과목이 이미 객체에 존재하는지 확인
        if (acc[subject]) {
          acc[subject] += 1; // 존재하면 갯수 증가
        } else {
          acc[subject] = 1; // 존재하지 않으면 새로 추가
        }
        return acc;
      }, {});
    console.table(subjectCount);
  })();
})();
